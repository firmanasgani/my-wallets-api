...
export class SubscriptionsService {
  ...
  async createCheckoutToken(userId: string, planCode: string) {
    ...
    try {
      ...
      this.logCheckoutToSheets({
        ...
        this.logger.warn(
          `[GoogleSheets] Failed to log checkout: ${err.message}`,
        ),
      );
  ...
  async handleMidtransWebhook(payload: any) {
    ...
    if (!signatureKey) {
      console.log(
        '[Midtrans Webhook] No signature key — treating as test ping, skipping processing.',
      );
      return { status: 'OK' };
    }
  ...
  async handleRecurringNotification(payload: any) {
    ...
    console.log(
      `[Recurring Notification] Order: ${orderId}, Status: ${transactionStatus}`,
    );

    if (!signatureKey) {
      console.log(
        `[Recurring Notification] No signature key — treating as test ping, skipping processing.`,
      );
      return { status: 'OK' };
    }
    ...
    if (serverSignature !== signatureKey) {
      console.warn(
        `[Recurring Notification] Invalid signature for orderId: ${orderId}`,
      );
      return { status: 'OK' };
    }
    ...
    console.log(
      `[Recurring Notification] No PaymentTransaction found for ${orderId}. Looking up user by email.`,
    );

    const userEmail = customerDetails?.email;
    if (!userEmail) {
      console.warn(
        `[Recurring Notification] No customer email in payload for orderId: ${orderId}`,
      );
      await this.logsService.create({
    ...
    if (!user) {
      console.warn(
        `[Recurring Notification] User not found for email: ${userEmail}`,
      );
      await this.logsService.create({
        actionType: LogActionType.SUBSCRIPTION_WEBHOOK,
        entityType: 'RecurringPayment',
    ...
    if (!activePlan) {
      console.warn(
        `[Recurring Notification] No active plan for user: ${user.id}`,
      );
      await this.logsService.create({
        userId: user.id,
        actionType: LogActionType.SUBSCRIPTION_WEBHOOK,
  ...
  async expirePendingPayments() {
    ...
    this.logger.log(
      `[ExpirePendingPayments] Marking PENDING payments older than ${oneDayAgo.toISOString()} as FAILED.`,
    );
    ...
    if (stale.length === 0) {
      this.logger.log(
        '[ExpirePendingPayments] No stale pending payments found.',
      );
      return;
    }
    ...
    this.logger.log(
      `[ExpirePendingPayments] Marked ${ids.length} payment(s) as FAILED: ${ids.join(', ')}`,
    );
  }
  ...
  async expireSubscriptions() {
    ...
    this.logger.log(
      `[ExpireSubscriptions] Running at ${now.toISOString()}. Checking endDate in range [${startOfYesterday.toISOString()}, ${startOfToday.toISOString()})`,
    );
    ...
    this.logger.log(
      `[ExpireSubscriptions] Expired ${ids.length} subscription(s): ${ids.join(', ')}`,
    );
  }
}
